...
describe("DocumentStoreCreator", async () => {
  ...
  describe("deploy", () => {
    it("should deploy new instance of DocumentStore correctly", async () => {
      ...
      expect(receipt.events[4].args[1]).to.be.equal(Accounts[0].address, "Emitted contract creator does not match");
      const deployedDocumentStore = await DocumentStore.attach(receipt.events[4].args[0]);
